...
class TheScene extends THREE.Scene {
  ...
  createLights () {
    ...
    // add spotlight for the shadows
    // luz roja con sombras
    this.spotLightExtra = new THREE.SpotLight( 0xff0000 );
    this.spotLightExtra.position.set( 0, 60, 40 );
    this.spotLightExtra.castShadow = true;

    // the shadow resolution
    this.spotLightExtra.shadow.mapSize.width=2048
    this.spotLightExtra.shadow.mapSize.height=2048;
    this.add (this.spotLightExtra);
  }
  ...
  animate (controls) {
    ...
    if(controls.extraLight == true){
      this.spotLightExtra.intensity = 0.5;
    }
    else{
      this.spotLightExtra.intensity = 0;
    }
    this.spotLight.intensity = controls.lightIntensity;
    this.crane.setHookPosition (controls.rotation, controls.distance, controls.height);
    
    var box = this.crane.box;
    //this.boxDropped = false;
    if(controls.height >= 20 && box !== null){
     this.dropBox();
     //boxDropped = true;
    }
  }
